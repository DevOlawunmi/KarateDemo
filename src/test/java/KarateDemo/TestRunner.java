...
/*
 ...
 they can send LBA chasers and they can search for existing cases. For these processes to run correctly, we send API calls
  to our database backend and to other web services like the DVLA API to get details of the vehicles involved in the incident,
   Address Lookup API, and Insurers API. In addition to this, ping authenticates users trying to login to the portal so that's another API
   call being made from the portal.
 */
